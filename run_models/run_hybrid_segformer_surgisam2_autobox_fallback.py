...
from Hybrid.surgisam2_fallback_inference import (
    ...
    predict_surgisam2_candidates_from_box,
    select_candidate_by_segformer_agreement,
)
...
METHOD_FOLDER_NAME = "SegFormer_SurgiSAM2_AutoBox_Fallback"
TRAINING_STATE_FOLDER = "hybrid"

# New tuned fallback prompt folder.
# This avoids overwriting the old weaker IoU-0.50 fallback results.
PROMPT_MODE_FOLDER = "Auto_box_fallback_dice_0p85_area_0p70_1p30"


# Debug first. For full run, use all datasets/splits and MAX_IMAGES_DEBUG = None.
DATASETS_TO_RUN = [
    "ENID",
    "GLENDA",
    "GLENDA_clean",
...
# e.g 5 for test debug run
MAX_IMAGES_DEBUG = None


# Multi-lesion/component prompt settings.
BOX_PADDING_PX = 0
BOX_PADDING_RATIO = 0.0

# If too many tiny false-positive boxes appear, try 20, 50, or 100.
BOX_MIN_COMPONENT_AREA_PX = 0

# None = use all SegFormer connected components.
BOX_MAX_COMPONENTS = None


# =============================================================================
# Tuned fallback acceptance settings
# =============================================================================
# IoU is kept as an additional safety check and for logging.
# Dice is now the main acceptance criterion.
ACCEPTANCE_IOU_THRESHOLD = 0.50
ACCEPTANCE_DICE_THRESHOLD = 0.85

# Prevent SurgiSAM2 from replacing a SegFormer component with a much smaller
# or much larger mask.
MIN_AREA_RATIO = 0.70
MAX_AREA_RATIO = 1.30

# Even when accepted, clip SurgiSAM2 to a small neighborhood around the original
# SegFormer component to prevent expansion into wrong tissue.
CLIP_ACCEPTED_MASK_TO_DILATED_SEGFORMER = True
...
def save_run_config(output_dir: Path):
    config = {
        ...
        "acceptance_iou_threshold": ACCEPTANCE_IOU_THRESHOLD,
        "acceptance_dice_threshold": ACCEPTANCE_DICE_THRESHOLD,
        "min_area_ratio": MIN_AREA_RATIO,
        "max_area_ratio": MAX_AREA_RATIO,
        "clip_accepted_mask_to_dilated_segformer": CLIP_ACCEPTED_MASK_TO_DILATED_SEGFORMER,
        ...
        "fallback_rule": (
            "For each exact SegFormer connected component, create one tight box, "
            "run SurgiSAM2, choose the candidate with highest Dice agreement against that exact "
            "SegFormer component, accept it only if IoU >= threshold, Dice >= threshold, "
            "and area ratio is within the allowed range. If accepted, optionally clip the "
            "SurgiSAM2 candidate to a dilated SegFormer component. If rejected, keep the "
            "original SegFormer component."
        ),
    }
...
def run_dataset_split(
    ...
    print("\n" + "=" * 100)
    print(f"Running tuned exact-component fallback hybrid: {dataset_key} | {split_key}")
    print(f"Images: {len(image_files)}")
    print(f"Output: {output_dirs['root']}")
    print(f"IoU threshold: {ACCEPTANCE_IOU_THRESHOLD}")
    print(f"Dice threshold: {ACCEPTANCE_DICE_THRESHOLD}")
    print(f"Area ratio range: [{MIN_AREA_RATIO}, {MAX_AREA_RATIO}]")
    print(f"Clip accepted masks: {CLIP_ACCEPTED_MASK_TO_DILATED_SEGFORMER}")
    print(f"Dilation radius px: {DILATION_RADIUS_PX}")
    print("=" * 100)
    ...
    for image_index, image_path in enumerate(image_files, start=1):
        ...
        with Timer() as total_timer:
            ...
            with Timer() as surgisam2_timer:
                ...
                if len(boxes_xyxy) == 0:
                    ...
                    prompt_status = "multi_box_exact_component_tuned_fallback_prompt_used"
                    ...
                    for component_index, box_xyxy in enumerate(boxes_xyxy, start=1):
                        ...
                        candidate_masks, candidate_scores = predict_surgisam2_candidates_from_box(
                        ...
                        selection = select_candidate_by_segformer_agreement(
                            ...
                            min_area_ratio=MIN_AREA_RATIO,
                            max_area_ratio=MAX_AREA_RATIO,
                            clip_to_dilated_segformer=CLIP_ACCEPTED_MASK_TO_DILATED_SEGFORMER,
                            dilation_radius_px=DILATION_RADIUS_PX,
                        )

                        selected_component_mask = selection["selected_mask"]
                        ...
                        component_decision = {
                            ...
                            "accepted_surgisam2": bool(selection["accepted"]),
                            "decision": selection["decision"],
                            "selected_candidate_index": selection["selected_candidate_index"],
                            "selected_surgisam2_score": selection["selected_surgisam2_score"],
                            "best_agreement_iou_with_segformer_component": selection["best_agreement_iou"],
                            "best_agreement_dice_with_segformer_component": selection["best_agreement_dice"],
                            "best_area_ratio_candidate_over_segformer": selection.get("best_area_ratio", None),
                            "acceptance_iou_threshold": ACCEPTANCE_IOU_THRESHOLD,
                            "acceptance_dice_threshold": ACCEPTANCE_DICE_THRESHOLD,
                            "min_area_ratio": MIN_AREA_RATIO,
                            "max_area_ratio": MAX_AREA_RATIO,
                            "clip_accepted_mask_to_dilated_segformer": CLIP_ACCEPTED_MASK_TO_DILATED_SEGFORMER,
                            "dilation_radius_px": DILATION_RADIUS_PX,
                            "candidate_agreements": selection["candidate_agreements"],
        ...
        prompt_data = {
            ...
            "root_name": root_name,
            "prompt_type": "multi_tight_box_exact_component_tuned_fallback",
