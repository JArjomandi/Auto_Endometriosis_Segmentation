...
OUTPUT_XLSX = OUTPUT_DIR / "segformer_vs_hybrid_surgisam2_statistics.xlsx"


# =============================================================================
# Hybrid result folder settings
# =============================================================================

HYBRID_METHOD_FOLDER = "SegFormer_SurgiSAM2_AutoBox"
HYBRID_TRAINING_STATE = "hybrid"
HYBRID_PROMPT_MODE = "Auto_box"
...
# These columns are already saved in your hybrid metrics_image_level.csv.
# SegFormer columns = before SurgiSAM2 refinement.
# Hybrid columns = after SurgiSAM2 refinement.
METRIC_PAIRS = {
    "dice": {
...
def run_wilcoxon_paired(segformer_values, hybrid_values):
    ...
    Positive difference:
        hybrid improved over SegFormer.

    Negative difference:
        hybrid worsened compared with SegFormer.
    """
...
def decide_better(median_difference, mean_difference, adjusted_p):
    ...
    if adjusted_p >= ALPHA:
        if median_difference > 0:
            return "Hybrid numerically higher, not significant"
        if median_difference < 0:
            return "SegFormer numerically higher, not significant"
        return "No difference"

    if median_difference > 0:
        return "Hybrid significantly better"

    if median_difference < 0:
        return "SegFormer significantly better"

    if mean_difference > 0:
        return "Hybrid significantly better by mean difference"
        ...
        summary_rows.append(
            {
                ...
                "Hybrid mean": hybrid_summary["mean"],
                "Hybrid SD": hybrid_summary["std"],
                "Hybrid mean ± SD": format_mean_sd(
                    hybrid_summary["mean"],
                    hybrid_summary["std"],
                ),
                "Hybrid median": hybrid_summary["median"],
                "Hybrid Q1": hybrid_summary["q1"],
                "Hybrid Q3": hybrid_summary["q3"],
                "Hybrid median [Q1, Q3]": format_median_iqr(
                    hybrid_summary["median"],
                    hybrid_summary["q1"],
                    hybrid_summary["q3"],
                ),

                "Mean difference Hybrid-SegFormer": diff_summary["mean"],
                "SD difference": diff_summary["std"],
                "Median difference Hybrid-SegFormer": diff_summary["median"],
            ...
            image_level_rows.append(
                {
                    ...
                    "Hybrid": hybrid_value,
                    "Difference Hybrid-SegFormer": difference_value,
...
def apply_holm_within_dataset_split(summary_df: pd.DataFrame):
    ...
    # Force safe dtypes before assigning mixed values.
    ...
    for (dataset_key, split_key), group_df in summary_df.groupby(["Dataset", "Split"]):
        ...
        for index, adjusted_p in zip(group_indices, adjusted_values):
            ...
            median_difference = summary_df.loc[
                index,
                "Median difference Hybrid-SegFormer",
            ]

            mean_difference = summary_df.loc[
                index,
                "Mean difference Hybrid-SegFormer",
            ]
    ...
    rows.append(
        {
            "Item": "Pairing",
            "Description": (
                "Paired by image. Each image has one SegFormer value and one "
                "hybrid value for the same metric."
            ),
        }
    )
    ...
    rows.append(
        {
            "Item": "Difference",
            "Description": (
                "Difference = Hybrid - SegFormer. Positive values mean the hybrid "
                "improved the metric. Negative values mean SurgiSAM2 refinement worsened it."
            ),
        }
    )
    ...
    rows.append(
        {
            ...
            "Description": (
                "Two-sided Wilcoxon signed-rank test, because the two methods are "
                "paired on the same images and segmentation metrics are bounded/skewed."
            ),
        }
    )
    ...
    rows.append(
        {
            "Item": "Decision rule",
            "Description": (
                "Hybrid is significantly better if median difference > 0 and "
            ...
            rows.append(
                {
                    ...
                    "Description": (
                        ...
                        f"Hybrid mean={row['Hybrid mean']:.4f}, "
                        f"median difference={row['Median difference Hybrid-SegFormer']:.4f}, "
...
def write_xlsx(summary_df, image_level_df, interpretation_df, config_df):
    ...
    with pd.ExcelWriter(OUTPUT_XLSX, engine="openpyxl") as writer:
        ...
        for row in range(2, summary_sheet.max_row + 1):
            ...
            if better_method == "Hybrid significantly better":
...
def main():
    ...
    if len(summary_df) == 0:
        raise RuntimeError(
            "No summary rows were created. Check that hybrid metrics_image_level.csv files exist."
        )
