...
def find_hybrid_masks(dataset_key: str, split: str, root_name: str):
    ...
    if final_mask_path is None:
        raise FileNotFoundError(
            f"Could not find final SurgiSAM2 hybrid mask for '{root_name}' in {final_mask_dir}"
        )
...
def plot_hybrid_example(dataset_key: str, split: str, root_name: str):
    ...
    panels = [
        ...
        {
            "title": "SurgiSAM2 Final",
            "bottom": format_dice(final_dice),
    ...
    output_png = (
        OUTPUT_ROOT
        / f"{dataset_key}_{split}_{root_name}_hybrid_segformer_surgisam2_5panel.png"
    )

    output_csv = (
        OUTPUT_ROOT
        / f"{dataset_key}_{split}_{root_name}_hybrid_segformer_surgisam2_5panel_files.csv"
    )
    ...
    loaded_row = {
        ...
        "surgisam2_final_dice": final_dice,
